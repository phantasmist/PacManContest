...
def runGames(layouts, agents, display, length, numGames, record, numTraining, redTeamName, blueTeamName, muteAgents=False, catchExceptions=False):
    ...
    if numGames > 1:
        ...
        print('Blue Win Rate: %d/%d (%.2f)' %
              ([s < 0 for s in scores].count(True), len(scores), redLoseRate))
        print('Record:       ', ', '.join(
...
def save_score(data, data_2):
    ...
    df_category_1 = pd.DataFrame(
        data=['<Average Winning Rate>'], columns=column_name, index=[''])
    df_category_2 = pd.DataFrame(
        data=['<Average Scores>'], columns=column_name, index=[''])

    df_1 = pd.DataFrame(data=data, columns=column_name)
    df_1.index = ['your_base1', 'your_base2', 'your_base3',
                  'baseline', 'Num_Win', 'Avg_Winning_Rate']

    df_2 = pd.DataFrame(data=data_2, columns=column_name)
    df_2.index = ['your_base1', 'your_base2',
                  'your_base3', 'baesline', 'Avg_Score']
...
if __name__ == '__main__':
    ...
    lst = ['your_baseline1.py', 'your_baseline2.py',
           'your_baseline3.py', 'baseline.py']
    for i in range(len(lst)):
        # Get game components based on input
        options = readCommand(sys.argv[1:], lst[i])
