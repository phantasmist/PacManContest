# your_baseline1.py
#
# added self.foodEaten so that offensiveReflexAgent runs back home
# by moving to starting point when the agent eats one food
...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        distToStart = self.getMazeDistance(
            successor.getAgentState(self.index).getPosition(), self.start)
    ...
    def getWeights(self, gameState, action):
        ...
        if self.foodEaten >= 1:
            return {'successorScore': 0, 'distanceToFood': 0, 'distToStart': -100}
        else:
            return {'successorScore': 100, 'distanceToFood': -1, 'distToStart': 0}
